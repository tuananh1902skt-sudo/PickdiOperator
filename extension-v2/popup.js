// Phải khớp DELAY_CONFIG_STORAGE_KEY/DEFAULT_DELAY_CONFIG bên background.js — không import
// chung được vì popup.js và background.js (service worker) chạy 2 context riêng biệt.
...
const DEFAULT_DELAY_CONFIG = {
  ...
  longDelayMinSec: 15,
  longDelayMaxSec: 45,
  longDelayChancePct: 40,
  breakEveryMin: 15,
  breakEveryMax: 20,
  breakDurationMinMin: 5,
  breakDurationMaxMin: 15,
  maxConsecutiveFailures: 3,
};
const DELAY_CONFIG_FIELD_IDS = {
  shortDelayMinSec: 'delayShortMin',
  shortDelayMaxSec: 'delayShortMax',
  longDelayMinSec: 'delayLongMin',
  longDelayMaxSec: 'delayLongMax',
  longDelayChancePct: 'delayLongChancePct',
  breakEveryMin: 'breakEveryMin',
  breakEveryMax: 'breakEveryMax',
  breakDurationMinMin: 'breakDurationMinMin',
  breakDurationMaxMin: 'breakDurationMaxMin',
  maxConsecutiveFailures: 'maxConsecutiveFailures',
};

// Tự động lưu và load Webapp URL + filter đã nhập gần nhất
document.addEventListener('DOMContentLoaded', () => {
  chrome.storage.local.get(['webappUrl', 'lastFilters', 'lastShopId', 'lastShopRegion', DELAY_CONFIG_STORAGE_KEY], (res) => {
    ...
    if (res.lastShopId) document.getElementById('manualShopId').value = res.lastShopId;
    document.getElementById('manualShopRegion').value = res.lastShopRegion || 'US';

    const delayConfig = { ...DEFAULT_DELAY_CONFIG, ...(res[DELAY_CONFIG_STORAGE_KEY] || {}) };
    Object.keys(DELAY_CONFIG_FIELD_IDS).forEach((key) => {
      document.getElementById(DELAY_CONFIG_FIELD_IDS[key]).value = delayConfig[key];
    });
  });
});

document.getElementById('saveShopIdBtn').addEventListener('click', async () => {
  ...
  const statusEl = document.getElementById('shopIdStatus');
  if (!shopId) {
    statusEl.textContent = '⚠️ Chưa nhập Shop ID.';
    return;
  }
  await chrome.storage.local.set({ lastShopId: shopId, lastShopRegion: shopRegion });
  statusEl.textContent = `✅ Đã lưu Shop ID: ${shopId} (${shopRegion})`;
});

document.getElementById('saveDelayConfigBtn').addEventListener('click', async () => {
  const statusEl = document.getElementById('delayConfigStatus');
  const config = {};
  for (const key of Object.keys(DELAY_CONFIG_FIELD_IDS)) {
    const raw = document.getElementById(DELAY_CONFIG_FIELD_IDS[key]).value;
    const num = Number(raw);
    config[key] = raw === '' || Number.isNaN(num) ? DEFAULT_DELAY_CONFIG[key] : num;
  }
  if (config.shortDelayMaxSec < config.shortDelayMinSec || config.longDelayMaxSec < config.longDelayMinSec || config.breakEveryMax < config.breakEveryMin || config.breakDurationMaxMin < config.breakDurationMinMin) {
    statusEl.textContent = '⚠️ Giá trị "max" phải ≥ giá trị "min" tương ứng.';
    return;
  }
  await chrome.storage.local.set({ [DELAY_CONFIG_STORAGE_KEY]: config });
  statusEl.textContent = '✅ Đã lưu cấu hình delay — áp dụng cho lượt cào tiếp theo (cả nút trong webapp).';
});
