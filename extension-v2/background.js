...
// Cấu hình "cơ chế delay" chống bị TCM phát hiện là bot — user chỉnh trong popup (mục Shop ID
// trở xuống), lưu ở đây để cả 2 nguồn kích hoạt hàng đợi (popup lẫn nút trong webapp) đều dùng
// chung 1 cấu hình duy nhất thay vì mỗi nơi 1 giá trị khác nhau.
const DELAY_CONFIG_STORAGE_KEY = 'tcmDelayConfig';
const DEFAULT_DELAY_CONFIG = {
  shortDelayMinSec: 5,
  shortDelayMaxSec: 15,
  longDelayMinSec: 15,
  longDelayMaxSec: 45,
  longDelayChancePct: 40, // % số lần dùng delay dài thay vì ngắn
  breakEveryMin: 15, // nghỉ dài sau mỗi 15-20 creator
  breakEveryMax: 20,
  breakDurationMinMin: 5, // nghỉ dài kéo dài 5-15 phút
  breakDurationMaxMin: 15,
  maxConsecutiveFailures: 3, // tự dừng queue nếu lỗi liên tiếp quá số này
};

async function getDelayConfig() {
  const res = await chrome.storage.local.get([DELAY_CONFIG_STORAGE_KEY]);
  return { ...DEFAULT_DELAY_CONFIG, ...(res[DELAY_CONFIG_STORAGE_KEY] || {}) };
}
...
// 60% (mặc định) rơi vào delay ngắn 5-15s, 40% rơi vào delay dài 15-45s — tránh pattern "luôn
// trong 1 khoảng hẹp" dễ bị nhận diện là bot so với chỉ random đều 1 khoảng cố định.
function pickItemDelayMs(state) {
  const useLong = Math.random() * 100 < (state.longDelayChancePct ?? DEFAULT_DELAY_CONFIG.longDelayChancePct);
  return useLong
    ? randomDelay((state.longDelayMinSec ?? DEFAULT_DELAY_CONFIG.longDelayMinSec) * 1000, (state.longDelayMaxSec ?? DEFAULT_DELAY_CONFIG.longDelayMaxSec) * 1000)
    : randomDelay((state.shortDelayMinSec ?? DEFAULT_DELAY_CONFIG.shortDelayMinSec) * 1000, (state.shortDelayMaxSec ?? DEFAULT_DELAY_CONFIG.shortDelayMaxSec) * 1000);
}
...
// HÀM CHẠY BÊN TRONG TAB TCM ẨN — không được closure biến ngoài (executeScript chỉ inject
// nguyên văn đúng hàm này, không mang theo các hàm khác trong file). Cuộn người thật + tự bấm
// hết mọi nút Retry trước khi đọc, vì trang chi tiết hay báo "Failed to load data" — xem cùng
// logic lặp lại ở shared.js (readTcmLastProfile/autoScanAndReadTcmProfile).
async function readProfileByCidInPage(cid) {
...
async function runLoop() {
  ...
  try {
    for (;;) {
      ...
      // Đang trong giờ nghỉ dài định kỳ (xem nhánh "nghỉ dài" bên dưới) — dừng vòng lặp ở đây,
      // BREAK_ALARM sẽ tự đánh thức runLoop() lại đúng giờ dù service worker có bị Chrome tắt
      // giữa chừng lúc idle hay không.
      ...
      if (state.index >= state.queue.length) {
        ...
        // nhiều so với vài giây giữa 2 creator (mặc định 45s) trước khi tự nạp chunk kế tiếp,
        // giống hành vi "nghỉ giải lao" của người dùng thật hơn là dừng hẳn rồi chạy lại ngay.
        // Nếu không bật auto-continue, dừng ở 'done' và giữ nguyên `pending` để user tự bấm
      ...
      const itemsSinceBreak = (fresh.itemsSinceBreak || 0) + 1;
      const maxConsecutiveFailures = fresh.maxConsecutiveFailures ?? DEFAULT_DELAY_CONFIG.maxConsecutiveFailures;

      // Nhiều creator liên tiếp đều lỗi/phải retry là dấu hiệu TCM đang để ý (rate-limit/risk
      // control) hơn là lỗi ngẫu nhiên — tự dừng hẳn và báo rõ lý do thay vì cố chạy tiếp.
      if (maxConsecutiveFailures > 0 && consecutiveFailures >= maxConsecutiveFailures) {
        await patchState({
          index: fresh.index + 1,
          results,
          processedCount: (fresh.processedCount || 0) + 1,
          failedCount: (fresh.failedCount || 0) + 1,
          currentHandle: null,
          consecutiveFailures,
          itemsSinceBreak,
          status: 'stopped',
          autoContinue: false,
          stoppedReason: `⚠️ Tự dừng sau ${consecutiveFailures} creator liên tiếp lỗi — có thể TCM đang chặn/giới hạn, kiểm tra lại trước khi chạy tiếp.`,
        });
      ...
      // Nghỉ dài định kỳ sau mỗi N creator (giống người thật lướt 1 lúc rồi làm việc khác) thay
      // vì chỉ delay ngắn đều đặn suốt — N và thời lượng nghỉ đều random trong khoảng cấu hình.
      const breakThreshold = after.nextBreakThreshold
        ?? randInt(after.breakEveryMin ?? DEFAULT_DELAY_CONFIG.breakEveryMin, after.breakEveryMax ?? DEFAULT_DELAY_CONFIG.breakEveryMax);
      if (breakThreshold > 0 && itemsSinceBreak >= breakThreshold) {
        const breakMs = randomDelay(
          (after.breakDurationMinMin ?? DEFAULT_DELAY_CONFIG.breakDurationMinMin) * 60000,
          (after.breakDurationMaxMin ?? DEFAULT_DELAY_CONFIG.breakDurationMaxMin) * 60000
        );
        await patchState({
          itemsSinceBreak: 0,
          onBreakUntil: Date.now() + breakMs,
          nextBreakThreshold: randInt(after.breakEveryMin ?? DEFAULT_DELAY_CONFIG.breakEveryMin, after.breakEveryMax ?? DEFAULT_DELAY_CONFIG.breakEveryMax),
        });
        chrome.alarms.create(BREAK_ALARM, { delayInMinutes: Math.max(breakMs / 60000, 0.5) });
        break;
      }
...
async function startAutoDetailQueueInternal(items, webappUrl, shopId, shopRegion, maxCount, autoContinue, cooldownMs) {
  ...
  const delayConfig = await getDelayConfig();
  await setState({
    ...
    onBreakUntil: null,
    nextBreakThreshold: randInt(delayConfig.breakEveryMin, delayConfig.breakEveryMax),
    stoppedReason: null,
...
async function continueAutoDetailQueueInternal() {
  ...
  await patchState({
    status: 'running', queue: nextQueue, pending: nextPending, index: 0, currentHandle: null,
    itemsSinceBreak: 0, consecutiveFailures: 0, onBreakUntil: null, stoppedReason: null,
  });
  runLoop();
...
async function runListImportJob(message) {
  ...
  try {
    ...
    // Luôn lưu mỗi lần Import (không chỉ khi tick auto-detail), vì đây là nơi duy nhất user
    // được hướng dẫn bấm để extension "nhớ" Shop ID.
...
async function runDetailJob(message) {
  ...
  stoppedJobTypes.delete(jobType);
  try {
    await setJob(jobType, { status: 'running', message: message.mode === 'scan' ? '⏳ Đang cuộn trang và tự động click qua các tab (có thể lâu hơn nếu phải auto-retry lỗi load)...' : '⏳ Đang đọc data đã bắt được...' });
    ...
    // Đã retry những tab thiếu data ở shared.js rồi — nếu sau đó vẫn còn thiếu thì đây là thiếu
    // THẬT (không phải lỗi thoáng qua), phải báo ⚠️ rõ ràng thay vì ✅ để không bị hiểu nhầm là
    // đã quét đủ toàn bộ chỉ số.
    const stillMissing = scraped.clickedButNoData && scraped.clickedButNoData.length > 0;
    const statusIcon = stillMissing ? '⚠️' : '✅';
    await setJob(jobType, { status: 'done', message: `${statusIcon} Đã cập nhật chi tiết cho @${detail.handle}. ${summary} ${tabReport}` });
  } catch (err) {
    await setJob(jobType, { status: 'error', message: `❌ ${String((err && err.message) || err)}` });
  }
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === 'STOP_AUTO_DETAIL_QUEUE') {
    (async () => {
      chrome.alarms.clear(AUTO_CONTINUE_ALARM);
      chrome.alarms.clear(BREAK_ALARM);
      await patchState({ status: 'stopped', autoContinue: false, onBreakUntil: null });
